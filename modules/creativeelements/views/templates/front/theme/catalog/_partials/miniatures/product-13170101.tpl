        ...
        <div data-elementor-type="product-miniature" data-elementor-id="13170101" class="elementor elementor-13170101{if !empty($productClasses)} {$productClasses}{/if}">
            <article class="elementor-section-wrap" data-id-product="{$product.id_product}" data-id-product-attribute="{$product.id_product_attribute}">
                    <section class="elementor-element elementor-element-ebdfb72 elementor-section-boxed elementor-section-height-default elementor-section-height-default elementor-section elementor-top-section" data-id="ebdfb72" data-element_type="section" data-settings="{literal}{&quot;background_background&quot;:&quot;classic&quot;}{/literal}">
        ...
        <div class="elementor-widget-container">        {$cb = Context::getContext()->controller->getContainer()}
        {if $cb->has('product_comment_repository')}
            ...
            {if $pcr}            {$average_grade = Tools::ps_round($pcr->getAverageGrade($product.id, $pcm), 1)}
            <div class="ce-product-rating">
                        <div class="elementor-star-rating__wrapper">
                        <div class="elementor-star-rating" title="5/5">        {$floored_rating = $average_grade|intval}
        {for $stars=1 to 5}
            {if $stars <= $floored_rating}
                <i class="elementor-star-full">&#xF005;</i>
            {elseif $floored_rating + 1 === $stars}
                <i class="elementor-star-{10*($average_grade - $floored_rating)}">&#xF005;</i>
            {else}
        ...
        <div class="elementor-widget-container">{if $product.available_for_order}        <div class="elementor-button-wrapper">
            <a {if $product.add_to_cart_url}href="#ce-action=addToCart{if $product.minimal_quantity > 1}&amp;qty={$product.minimal_quantity}{/if}"{/if} class="elementor-button elementor-size-sm" role="button">
