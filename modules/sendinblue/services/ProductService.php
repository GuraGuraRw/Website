...
class ProductService
{
    ...
    public function getTotalProductCount()
    {
        $result = \DbCore::getInstance()->executeS(
            sprintf(
                'SELECT count(*)  AS `count` FROM  `%sproduct` p LEFT JOIN `%sproduct_lang` pl ON (pl.`id_product` = p.`id_product` AND pl.`id_lang` = %s AND pl.`id_shop` = %s ) JOIN `%sproduct_shop` sa ON (p.`id_product` = sa.`id_product` AND sa.id_shop = 1) WHERE (sa.`active` = 1 AND state = 1)',
                _DB_PREFIX_,
                _DB_PREFIX_,
                (int) $this->idLang,
                (int) $this->idShop,
