...
namespace {
    ...
    if (!class_exists('PhpEncryptionLegacyEngine')) {
        if (file_exists(($filename = $root_dir . '/classes/PhpEncryptionLegacyEngine.php')) || file_exists(($filename = dirname(__FILE__) . '/PhpEncryptionLegacyEngine.php')))
    ...
    class Upgrade
    {
        ...
        private function getSQLFiles()
        {
            ...
            if (!$this->hasFailure()) {
                foreach ($neededUpgradeFiles as $version) {
                    ...
                    if (!$sqlContent = Tools::file_get_contents($file)) {
                        $this->logError('Error while loading SQL upgrade file "%file%.sql".', 33, ['%file%' => $version]);
                    }
                    $sqlContent .= "\n";
        ...
        private function updateHtaccess()
        {
            if (!class_exists('\Tools2', false) && class_exists('\ToolsCore')) {
                call_user_func('eval', 'class Tools2 extends \ToolsCore{}');
            }
        ...
        public static function migrateSettingsFile(Event $event = null)
        {
            ...
            $fileMigrated = false;
            if (!$addNewCookieKey) {
                $default_parameters = Yaml::parse(Tools::file_get_contents($root_dir . '/app/config/parameters.yml.dist'));
            }
            ...
            if (file_exists($root_dir . '/' . self::SETTINGS_FILE)) {
                $tmp_settings = Tools::file_get_contents($root_dir . '/' . self::SETTINGS_FILE);
            } else {
                $tmp_settings = null;
            }
            ...
            if (!file_exists($root_dir . '/app/config/parameters.yml') && $tmp_settings && strpos($tmp_settings, '_DB_SERVER_') !== false) {
                ...
                $parameters = Yaml::parse(Tools::file_get_contents($root_dir . '/app/config/parameters.yml'));
