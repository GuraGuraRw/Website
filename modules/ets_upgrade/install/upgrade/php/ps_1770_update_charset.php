...
{
    $adminFilterTableExists = $adminFilterFilterIdExists = $moduleHistoryTableExists = $translationTableExists = $adminFilterFilterIdNew = false;

    if (Db::getInstance()->executeS('SHOW TABLES LIKE \'' . _DB_PREFIX_ . 'admin_filter\'')) {
        $column_exist_filter_id = Db::getInstance()->executeS('SHOW COLUMNS FROM `' . _DB_PREFIX_ . 'admin_filter` LIKE \'filter_id\'');
        if (!$column_exist_filter_id) {
            $column_exist_filter_id = $adminFilterFilterIdNew = Db::getInstance()->execute('ALTER TABLE `' . _DB_PREFIX_ . 'admin_filter` ADD `filter_id` VARCHAR(191) NOT NULL');
        }
        if ($column_exist_filter_id && !Db::getInstance()->executeS('SHOW INDEX FROM `' . _DB_PREFIX_ . 'admin_filter` WHERE `Key_name`=\'admin_filter_search_id_idx\' AND `Column_name`=\'filter_id\'')) {
            Db::getInstance()->execute('ALTER TABLE `' . _DB_PREFIX_ . 'admin_filter` ADD UNIQUE INDEX admin_filter_search_id_idx(`employee`,`shop`,`controller`,`action`,`filter_id`)');
        }
    }
    ...
    if ($adminFilterTableExists) {
        if ($adminFilterFilterIdExists && !$adminFilterFilterIdNew) {
