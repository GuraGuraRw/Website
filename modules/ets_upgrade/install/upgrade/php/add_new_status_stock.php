...
function add_new_status_stock()
{
    ...
    if (!$count) {
        if (!Db::getInstance()->executeS('SHOW COLUMNS FROM `' . _DB_PREFIX_ . 'tab` LIKE \'hide_host_mode\';')) {
            Db::getInstance()->execute('ALTER TABLE `' . _DB_PREFIX_ . 'tab` ADD `hide_host_mode` tinyint(1) NOT NULL DEFAULT \'0\' AFTER  `active`;');
        }
