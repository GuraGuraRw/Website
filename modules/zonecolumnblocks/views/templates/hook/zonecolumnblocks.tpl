  ...
  {foreach from=$columnBlocks item=block name=columnBlocks}
    <div class="column-block md-bottom clearfix {$block.custom_class}">
      {if $block.block_type == $blocktype_product }
        <p class="column-title">{$block.title}</p>

        {if $block.enable_slider}
          {include file="module:zonecolumnblocks/views/templates/hook/product-list-slider.tpl" ablock=$block}
        {else}
          {include file="module:zonecolumnblocks/views/templates/hook/product-list.tpl" ablock=$block}
        {/if}
