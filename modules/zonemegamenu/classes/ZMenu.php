    ...
    public static function getListFront($title_image_url = '', $title_image_local = '')
    {
        ...
        if ($result) {
            ...
            foreach ($result as $menu) {
                ...
                if (isset($total_dropdown[$menu['id_zmenu']]) && $menu['drop_column']) {
                    $menu['dropdowns'] = $total_dropdown[$menu['id_zmenu']];
                }

                if (preg_match('/^(#[a-f0-9]{3}([a-f0-9]{3})?)$/i', $menu['label_color'])) {
                    $menu['label_color'] = 'background-color:'.$menu['label_color'].';';
                } else {
