...
{block name="input"}
	...
	<div id="{$input.id|escape:'htmlall':'UTF-8'}">
		{assign var=input_name value=$input.name}
		{assign var=field_array value=$fields_value.$input_name}
		{foreach from=$input.list_manufacturer key=manu_id item=manu_name}
		<div class="checkbox">
			<label>
				<input type="checkbox" name="{$input.name|escape:'htmlall':'UTF-8'}[]" value="{$manu_id|escape:'html':'UTF-8'}" {if $manu_id|in_array:$field_array}checked="checked"{/if} >
