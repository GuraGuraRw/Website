...
class ZProductExtraField extends ObjectModel
{
    ...
    public static function getSuppliersByIdField($id_zproduct_extra_field = 0)
    {
        if ($id_zproduct_extra_field == 0) {
            return array();
        }
        ...
        $product_extrafield = new self($id_zproduct_extra_field);
        if ($product_extrafield->suppliers != '') {
            return array_map('intval', explode(',', $product_extrafield->suppliers));
        } else {
            return array();
        }
    }

    public static function getFeaturesByIdField($id_zproduct_extra_field = 0)
    {
        if ($id_zproduct_extra_field == 0) {
            return array();
        }
        ...
        $product_extrafield = new self($id_zproduct_extra_field);
        if ($product_extrafield->features != '') {
            return array_map('intval', explode(',', $product_extrafield->features));
        } else {
            return array();
        }
    }
    ...
    public static function getSupplierIdsByProductId($id_product)
    {
        $query = 'SELECT DISTINCT `id_supplier` FROM `'._DB_PREFIX_.'product_supplier`
    ...
    public static function getFeatureIdsByProductId($id_product)
    {
        $query = 'SELECT DISTINCT `id_feature_value` FROM `' . _DB_PREFIX_ . 'feature_product`
                WHERE `id_product` = ' . (int) $id_product;
    ...
    public static function getFieldsByProductId($id_product, $hook)
    {
        ...
        $id_suppliers = self::getSupplierIdsByProductId($id_product);
        $id_features = self::getFeatureIdsByProductId($id_product);
        $results = array();
            ...
            if ($row['hook'] == $hook) {
                switch ($row['scope']) {
                    ...
                    case 'Specific Manufacturers':
                        ...
                        break;
                    case 'Specific Suppliers':
                        if (array_intersect(explode(',', $row['suppliers']), $id_suppliers)) {
                            $results[] = $row;
                        }
                        break;
                    case 'Specific Features':
                        if (array_intersect(explode(',', $row['features']), $id_features)) {
                            $results[] = $row;
                        }
                        break;
                }
