...
$twig_vars = array(
    ...
    'imageDir' => rtrim(_PS_BASE_URI_, '/') . '/modules/ets_upgrade/views/img/',
    'md5KeyCheck' => @file_exists($keyFile = $ps_root_path . '/modules/ets_upgrade/cache/key.txt') ? $keyFile : false,
    'linkBo' => $linkBo,
    'linkFo' => $currentFo,
    'submitKey' => ($postKey = Tools14::getValue('key')),
);

// Check & Validate MD5 key code.
if (Tools14::isSubmit('ets_ug_submit')) {
    if (!$postKey) {
        $errorMessage = $translator->trans('Rollback key is required.', array(), 'Module.Etsupgrade.Admin');
    } else {
        $keyCheckMD5 = trim(Tools14::file_get_contents($ps_root_path . '/modules/ets_upgrade/cache/key.txt'));
        if (!preg_match('/^[a-f0-9]{32}$/', $postKey) || !trim($keyCheckMD5) || $postKey !== $keyCheckMD5) {
            $errorMessage = $translator->trans('Rollback key is invalid.', array(), 'Module.Etsupgrade.Admin');
        }
    }
}
...
if (!empty($errorMessage)) {
    ...
    $twig_vars['jsParams'] = $container->getFileConfigurationStorage()->load(UpgradeFileNames::FILES_INIT_JS_PARAMS);
    $twig_vars['availableBackups'] = (new BackupFinder($container->getProperty($container::BACKUP_PATH)))->getAvailableBackups();
}

// Display Rollback Form.
$twig = $container->getTwig();
echo $twig->render('@ModuleAutoUpgrade/block/rollback.twig', $twig_vars);