...
{else}
  ...
  <div class="form-group row {$field.name} {if !empty($field.errors)}has-error{/if}">
    <label class="col-12 col-lg-3 form-control-label{if $field.required} required{/if}{if $field.type == 'checkbox'} d-none d-md-block{/if}" for="field-{$field.name}">
      {if $field.type !== 'checkbox'}
        {$field.label}
      {/if}
    </label>
    <div class="col-12 js-input-column {if in_array($field.type, ['radio-buttons', 'checkbox'])}col-lg-9{else}col-lg-6{/if}">
      ...
      {elseif $field.type === 'radio-buttons'}
        {block name='form_field_item_radio'}
          {foreach from=$field.availableValues item="label" key="value"}
            <label class="custom-radio" for="field-{$field.name}-{$value}">
        ...
        {block name='form_field_item_password'}
          <div class="input-group js-parent-focus">
            <input
              ...
              {if !empty($field.autocomplete)}autocomplete="{$field.autocomplete}"{/if}
              value=""
      ...
      {else}
        {block name='form_field_item_other'}
          <input
            ...
            {if !empty($field.autocomplete)}autocomplete="{$field.autocomplete}"{/if}
            {if !empty($field.availableValues.placeholder)}placeholder="{$field.availableValues.placeholder}"{/if}
