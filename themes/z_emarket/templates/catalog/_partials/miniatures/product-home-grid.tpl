    ...
    <div class="second-third-block">
      <div class="second-block">
        ...
        {block name='product_availability'}
          {if !empty($modules.zonethememanager.product_grid_available) && $product.show_availability && $product.availability_message}
            <div class="product-availability">
              <span class="{$product.availability}{if isset($product.quantity_all_versions) && $product.quantity_all_versions <=0} alloutofstock{/if}{if $product.availability == 'available' && $product.quantity <= 0} available-order{/if}">{$product.availability_message}</span>
            </div>
          {/if}
        {/block}

        {block name='product_description_short'}
          {if !empty($modules.zonethememanager.product_grid_desc)}
        ...
        {block name='product_variants'}
          {if !empty($modules.zonethememanager.product_grid_colors) && $product.main_variants}
